...
public class Solution {
    ...
    public static int[] getProductsOfAllIntsExceptAtIndex(int[] a) {
        ...
        int[] memtill = new int[a.length];
        int[] memafter = new int[a.length];
        memtill[0] = 1;
        memafter[a.length - 1] = 1;
        for(int i = 1 ; i < a.length ; i++){
            memtill[i] = memtill[i-1] * a[i - 1];
            memafter[a.length - i - 1] = memafter[a.length - i] * a[a.length - i];
        }
        int[] res = new int[a.length];
        for(int i = 0 ; i < a.length ; i++){
            res[i] = memtill[i] * memafter[i];
        }
        return res;
    }
